...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_courses&task=courses.saveOrderAjax&tmpl=component';
			...
			<table class="table table-striped" id="courseList">
				<thead>
					<tr>
						...
						<?php if ($assoc): ?>
							<th width="5%" class="nowrap hidden-phone">
								<?php echo JHtml::_('searchtools.sort', 'COM_COURSES_HEADING_ASSOCIATION', 'association', $listDirn, $listOrder); ?>
							</th>
						<?php endif; ?>
						<th width="10%" class="nowrap hidden-phone">
				...
				<tbody>
					<?php foreach ($this->items as $i => $item):
						...
						$canEdit    = $user->authorise('core.edit',       'com_courses.course.' . $item->id);
						$canCheckin = $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
						$canEditOwn = $user->authorise('core.edit.own',   'com_courses.course.' . $item->id) && $item->created_by == $userId;
						$canChange  = $user->authorise('core.edit.state', 'com_courses.course.' . $item->id) && $canCheckin;
						?>
						<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
							...
							<td class="center">
								<div class="btn-group">
									...
									<?php echo JHtml::_('course.featured', $item->featured, $i, $canChange); ?>
									<?php
							...
							<td class="nowrap has-context">
								<div class="pull-left">
									...
									<?php if ($canEdit || $canEditOwn): ?>
										<a href="<?php echo JRoute::_('index.php?option=com_courses&task=course.edit&id=' . (int) $item->id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>"><?php echo $this->escape($item->title); ?></a>
									<?php else: ?>
							...
							<?php if ($assoc): ?>
								<td class="hidden-phone">
									<?php if ($item->association): ?>
										<?php echo JHtml::_('course.association', $item->id); ?>
									<?php endif; ?>
								</td>
							<?php endif; ?>
							<td class="small hidden-phone">
