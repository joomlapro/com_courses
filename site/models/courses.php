...
/**
 * This models supports retrieving lists of courses.
 *
 * @package     Courses
 * @subpackage  com_courses
 * @since       3.2
...
class CoursesModelCourses extends JModelList
{
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.course_id.include');
	...
	/**
	 * Get the master query for retrieving a list of courses subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.id, a.title, a.alias, a.date_start, a.date_end, a.vacancies, a.description'
		...
		// Process an Archived Course layout.
		if ($this->getState('filter.state') == 2)
		{
			// If badcats is not null, this means that the course is inside an archived category.
			// In this case, the state is set to 2 to indicate Archived (even if the course state is Published).
		...
		// Join over the frontpage courses.
		if ($this->context != 'com_courses.featured')
		{
			$query->join('LEFT', '#__courses_frontpage AS fp ON fp.course_id = a.id');
		}
		...
		$subquery .= 'WHERE parent.extension = ' . $db->quote('com_courses');
		...
		else
		{
			...
			// If all categories are published, badcats.id will be null, and we just use the course state.
		...
		{
			// Use course state if badcats.id is null, otherwise, force 0 for unpublished.
		...
		// Filter by a single or group of courses.
		$courseId = $this->getState('filter.course_id');

		if (is_numeric($courseId))
		{
			$type = $this->getState('filter.course_id.include', true) ? '= ' : '<> ';
			$query->where('a.id ' . $type . (int) $courseId);
		}
		elseif (is_array($courseId))
		{
			...
			$type = $this->getState('filter.course_id.include', true) ? 'IN' : 'NOT IN';
			$query->where('a.id ' . $type . ' (' . $courseId . ')');
		}
	...
	public function getItems()
	{
		...
		$globalParams = JComponentHelper::getParams('com_courses', true);

		// Convert the parameter fields into objects.
		foreach ($items as &$item)
		{
			$courseParams = new JRegistry;
			$courseParams->loadString($item->params);

			// Unpack readmore and layout params.
			$item->alternative_readmore = $courseParams->get('alternative_readmore');
			$item->layout = $courseParams->get('layout');
			$item->params = clone $this->getState('params');

			/*
			For blogs, course params override menu item params only if menu param = 'use_course'.
			Otherwise, menu item params control the layout.
			If menu item is 'use_course' and there is no course param, use global.
			*/
			if ($input->getString('option') == 'com_courses' && (($input->getString('layout') == 'blog') || ($input->getString('view') == 'featured') || ($this->getState('params')->get('layout_type') == 'blog')))
			{
				// Create an array of just the params set to 'use_course'.
				...
				foreach ($menuParamsArray as $key => $value)
				{
					if ($value === 'use_course')
					{
						// If the course has a value, use it.
						if ($courseParams->get($key) != '')
						{
							// Get the value from the course.
							$courseArray[$key] = $courseParams->get($key);
						}
						else
						{
							// Otherwise, use the global value.
							$courseArray[$key] = $globalParams->get($key);
						}
					}
				}

				// Merge the selected course params.
				if (count($courseArray) > 0)
				{
					$courseParams = new JRegistry;
					$courseParams->loadArray($courseArray);
					$item->params->merge($courseParams);
				}
			}
			else
			{
				// For non-blog layouts, merge all of the course params.
				$item->params->merge($courseParams);
			}
			...
			if (!$guest)
			{
				$asset = 'com_courses.course.' . $item->id;
			...
			if ($access)
			{
				// If the access filter has been set, we already have only the courses this user can view.
			...
			$item->tags->getItemTags('com_courses.course', $item->id);
		}
