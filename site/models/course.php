...
class CoursesModelCourse extends JModelItem
{
	 ...
	 * @var     string
	 */
	protected $_context = 'com_courses.course';
	 ...
	 *
	 * @param   integer  $pk  The id of the course.
	 *
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($data))
				{
					return JError::raiseError(404, JText::_('COM_COURSES_ERROR_COURSE_NOT_FOUND'));
				}
				...
				{
					$userId = $user->get('id');
					$asset = 'com_courses.course.' . $data->id;
	...
	/**
	 * Increment the hit counter for the course.
	 *
	 * @param   integer  $pk  Optional primary key of the course to increment.
	 *
	...
	public function hit($pk = 0)
	{
		...
		if ($hitcount)
		{
			...
			$table = JTable::getInstance('Course', 'CoursesTable');
			$table->load($pk);
			$table->hit($pk);
		}
