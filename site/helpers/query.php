...
/**
 * Courses Component Query Helper.
 *
 * @static
 * @package     Courses
 * @subpackage  com_courses
...
class CoursesHelperQuery
{
	 ...
	 *
	 * @param   JRegistry  $params  An options object for the course.
	 *
	...
	public static function buildVotingQuery($params = null)
	{
		...
		if ($voting)
		{
			...
			$join   = ' LEFT JOIN #__courses_rating AS v ON a.id = v.course_id';
		}
		else
		{
			$select = '';
	...
	/**
	 * Method to order the intro courses array for ordering
	 * down the columns instead of across.
	 * The layout always lays the introtext courses out across columns.
	 * Array is reordered so that, when courses are displayed in index order
	 * across columns in the layout, the result is that the
	 * desired course ordering is achieved down the columns.
	 *
	 * @param   array    &$courses   Array of intro text courses.
	 ...
	 * @since   3.2
	 */
	public static function orderDownColumns(& $courses, $numColumns = 1)
	{
		$count = count($courses);

		// Just return the same array if there is nothing to change.
		if ($numColumns == 1 || !is_array($courses) || $count <= $numColumns)
		{
			$return = $courses;
		}
		// We need to re-order the intro courses array.
		else
		{
			...
			// Layout the courses in column order, skipping empty cells.
			$i = 0;
			...
			// Now read the $index back row by row to get courses in right row/col
			...
			for ($row = 1; ($row <= $maxRows) && ($i < $count); $row++)
			{
				for ($col = 1; ($col <= $numColumns) && ($i < $count); $col++)
				{
					$return[$keys[$i]] = $courses[$index[$row][$col]];
